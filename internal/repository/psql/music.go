...
func (r *Musics) Create(ctx context.Context, music domain.Music) error {
	_, err := r.db.Exec("INSERT INTO musics (title, author, publish_date, rating) values ($1, $2, $3, $4)",
...
func (r *Musics) GetByID(ctx context.Context, id int64) (domain.Music, error) {
	...
	err := r.db.QueryRow("SELECT id, title, author, publish_date, rating FROM musics WHERE id=$1", id).
