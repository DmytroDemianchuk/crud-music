...
func (m *Musics) Create(ctx context.Context, music domain.Music) error {
	_, err := m.db.Exec("INSERT INTO musics (name, artist, publish_date, genre) values ($1, $2, $3, $4)",
...
func (m *Musics) GetByID(ctx context.Context, id int64) (domain.Music, error) {
	...
	err := m.db.QueryRow("SELECT id, name, artist, publish_date, genre FROM musics WHERE id=$1", id).
...
func (m *Musics) GetAll(ctx context.Context) ([]domain.Book, error) {
	rows, err := m.db.Query("SELECT id, name, artist, publish_date, genre FROM musics")
	if err != nil {
		return nil, err
	}
